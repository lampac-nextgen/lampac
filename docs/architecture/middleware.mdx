...
<Steps>
  ...
  <Step title="Лимиты, статистика, контроллеры, RCH">
    Финальные middleware: лимитирование запросов, сбор статистики (`openstat`), маппинг контроллеров (`MapControllers`), RCH-эндпоинты (`/rch/result`, `/rch/gzresult`) и WebSocket `/ws`.
  </Step>
</Steps>
...
ForwardedHeaders
    ...
    → [/nws WebSocket]
    → Routing
    → Compression / Staticache
    → Ранний UseModule (EventListener.Middleware)
    → ProxyImg
    → StaticFiles
    → WAF
    → Authorization
    → Accsdb
    → Поздний UseModule
    → Лимиты / Статистика / Controllers / RCH
```
...
<Note>
  Благодаря горячей перезагрузке вы можете менять порты, включать или отключать модули, редактировать WAF-листы и лимиты без остановки сервера. Изменения применяются в течение 1 секунды после сохранения файла.
</Note>
